...
static size_t pipe_read(struct _open_file_descriptor *ofd, void* buf, size_t size);
static size_t pipe_write(struct _open_file_descriptor *ofd, const void* buf, size_t size);
...
static size_t pipe_read(struct _open_file_descriptor *ofd, void* buf, size_t size) {
	...
	struct extra_data_pipe_t *extra = (struct extra_data_pipe_t*)ofd->extra_data;
...
static size_t pipe_write(struct _open_file_descriptor *ofd, const void* buf, size_t size) {
	...
	while (extra->pos_read != extra->pos_write + 1 && c < size) {
		extra->data[extra->pos_read] = ((uint8_t*)buf)[c];
		c++;
